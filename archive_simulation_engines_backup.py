...
try:
    ...
    # trigger_builder 전용 샘플 데이터셋 경로
    SAMPLED_MARKET_DATA_DB_PATH = os.path.join(os.path.dirname(__file__), "..", "data", "sampled_market_data.sqlite3")
...
class RealDataSimulationEngine(BaseSimulationEngine):
    ...
    def __init__(self, data_db_path: str = None):
        ...
        if data_db_path is None:
            if USE_NEW_DB_PATHS:
                self.data_db_path = SAMPLED_MARKET_DATA_DB_PATH  # sampled_market_data.sqlite3로 매핑됨
                print(f"🔗 RealDataSimulationEngine: 새로운 통합 샘플 DB 사용 - {self.data_db_path}")
            else:
                # trigger_builder 전용 샘플 데이터셋 경로
                self.data_db_path = os.path.join(os.path.dirname(__file__), "..", "data", "sampled_market_data.sqlite3")
                print(f"⚠️ RealDataSimulationEngine: 레거시 샘플 DB 경로 사용 - {self.data_db_path}")
        else:
        ...
        self.current_scenario = None  # 현재 시나리오 추적
    ...
    def __init__(self, data_db_path: str = None):
        ...
        if data_db_path is None:
            if USE_NEW_DB_PATHS:
                ...
                # trigger_builder 전용 데이터셋 경로
                self.data_db_path = os.path.join(os.path.dirname(__file__), "..", "data", "market_data.sqlite3")
