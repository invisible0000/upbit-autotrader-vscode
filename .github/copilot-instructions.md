...
## 🔁 작업 프로세스 (에이전트 출력 계약)
1) 짧은 프리앰블 후 즉시 실행. 불필요한 질문 금지, 필수 질문 최대 3개
2) 기본 가정(답변 없을 시): 심볼 KRW-BTC, TF 1m/5m/15m, 수수료 0.05%, 슬리피지 1틱, 로컬 3-DB, dry-run
3) 출력 계약(순서 고정)
   - Plan: 목표·범위·의존성·리스크(간결)
   - Planned Changes: A/M/D · 파일경로 · 한줄요약
   - Diff: 파일별 unified diff
   - Tests: 테스트 코드 + pytest -q 실행
   - Verify & Rollback: 수동 검증 체크리스트 + 복구 절차

---

## 🧪 테스트·품질 게이트
- 테스트: pytest 필수, 최소 happy path + 1-2 edge
- 품질 게이트: Build/Lint/Unit/Smoke 보고 (PASS/FAIL), 변경 영향만 기록
- 검증 최종 단계: python run_desktop_ui.py 실행으로 UI/전략 무결성 확인
...
- [ ] pytest 테스트 포함 및 통과, UI 런 통해 7규칙 검증
