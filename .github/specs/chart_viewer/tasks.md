...
- [ ] 0.1 기존 InMemoryEventBus 호환성 확장
  - [ ] ChartViewerEvent 타입 추가 (기존 DomainEvent 상속)
  - [ ] ChartViewerPriority 상수 정의 (기존 priority 범위 내)
  - [ ] 기존 subscribe/publish 메서드 활용
  - [ ] 기존 시스템과 완전 격리된 차트뷰어 이벤트 처리
  - _검증: 기존 매매 시스템 영향 없음 확인_

- [ ] 0.2 창 상태 기반 우선순위 관리 시스템
  - [ ] ChartViewerResourceManager 구현 (기존 시스템 독립)
  - [ ] WindowLifecycleManager 구현 (기존 이벤트 버스 활용)
  - [ ] 창 상태별 우선순위 자동 조정 (5→8→10 범위)
  - [ ] 기존 시스템과 리소스 격리
  - _검증: 우선순위 변경이 기존 시스템에 영향 없음_

- [ ] 0.3 1개월 타임프레임 지원 시스템
  - [ ] TimeframeSupport 클래스 구현
  - [ ] 1w, 1M 타임프레임 API 전용 처리
  - [ ] WebSocket/API 하이브리드 모드 구현
  - [ ] 기존 타임프레임과 호환성 보장
  - _검증: 모든 타임프레임 (1m~1M) 데이터 수집 가능_
...
- [ ] 0.4 기존 시스템 안전성 검증
  ...
  - [ ] 이벤트 처리 성능 영향 분석
  - [ ] 기존 기능 동작 무결성 확인
  - _검증: 기존 시스템 100% 정상 동작 보장_

### Phase 1: 마켓 데이터 백본 구축 (3-4일)

- [ ] 1.1 기존 호환 데이터 수집 엔진 구현
  - [ ] 기존 이벤트 시스템 활용한 MarketDataEventHandler
  - [ ] 멀티소스 수집기 (API + WebSocket, 1개월 포함)
  - [ ] 데이터 검증 및 중복 제거 로직
  - [ ] 타임프레임 변환 엔진 (1분→1개월)
  - [ ] 기존 DomainEvent 상속 이벤트 발행
  - _검증: 1분 데이터로 1개월 캔들 생성 + 기존 시스템 영향 없음_

- [ ] 1.2 기존 호환 메모리 캐시 시스템 구현
  - [ ] 기존 인프라와 호환되는 캐시 시스템
  - [ ] LRU 정책 기반 메모리 관리 (차트뷰어 전용 할당)
  - [ ] 압축 저장 및 빠른 범위 검색
  - [ ] 기존 시스템과 메모리 격리
  - _검증: 캐시 성능 향상, 기존 시스템 메모리 영향 없음_

- [ ] 1.3 기존 호환 구독 관리 시스템 구현
  - [ ] 기존 InMemoryEventBus 구독 시스템 활용
  - [ ] 차트뷰별 독립적 데이터 구독/해제
  - [ ] 기존 priority 시스템과 호환되는 우선순위 관리
  - [ ] 중복 구독 방지 및 리소스 공유
  - _검증: 기존 구독과 격리된 차트뷰 구독/해제_

- [ ] 1.4 DB 통합 및 영구 저장 (기존 호환)
  - [ ] 기존 `data/market_data.sqlite3` 활용
  - [ ] 1개월 캔들 데이터 스키마 확장
  - [ ] 기존 히스토리 데이터와 호환성 보장
  - [ ] 실시간 데이터와 히스토리 동기화
  - _검증: 1개월 캔들 포함 모든 데이터 1초 내 로딩_
...
- [ ] 차트뷰어 우선순위(5,8,10)가 기존 우선순위(1-3)와 격리됨
- [ ] 1개월 타임프레임 포함 모든 데이터 수집 가능
- [ ] 기존 매매 엔진 100% 정상 동작 보장

### Phase 1 완료 기준
- [ ] 1분 데이터로 1개월 캔들 변환 + 기존 시스템 영향 없음
- [ ] 캐시 성능 향상, 기존 메모리 사용량 영향 없음
- [ ] 기존 구독과 격리된 차트뷰 구독/해제 동작
- [ ] 1개월 캔들 포함 모든 데이터 1초 내 로딩
