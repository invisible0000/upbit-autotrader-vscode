...
### 1. 전체 레이아웃 (1:4:2 비율 3열 구조)
```
┌──────────┬──────────────────────────────────────────────────┬─────────────┐
│          │                  Chart Area (4 비율)             │             │
│  Coin    │  ┌────────────────────────────────────────────┐  │  Orderbook  │
│  List    │  │         Main Plot (Candlestick)            │  │             │
│          │  │              (3 비율)                      │  │   (호가창)  │
│ (1 비율) │  ├────────────────────────────────────────────┤  │             │
│          │  │      Sub Plot (Volume/MACD/RSI/etc)        │  │  (2 비율)   │
│          │  │              (1 비율)                      │  │             │
│          │  └────────────────────────────────────────────┘  │             │
│          │                                                  │             │
└──────────┴──────────────────────────────────────────────────┴─────────────┘
...
### Phase 0: 기존 이벤트 버스 확장 (1-2일)
- [ ] 기존 InMemoryEventBus와 호환성 검증
- [ ] ChartViewerEvent 타입 추가 (우선순위 5,8,10)
- [ ] 1개월 타임프레임 지원 추가
- [ ] 차트뷰어 전용 이벤트 핸들러 구현
...
### 4. 동적 레이아웃 시스템
- **3열 스플리터**: QSplitter 기반 1:4:2 비율 구현
- **최소 크기**: 각 패널별 최소 너비 보장
- **리사이즈 이벤트**: 창 크기 변경시 레이아웃 자동 조정
...
2. **기존 시스템 호환**: InMemoryEventBus 확장으로 매매 시스템 영향 없음
3. **마켓 데이터 백본**: 통합 데이터 관리 시스템 안정적 동작
4. **리소스 관리**: 창 비활성화시 리소스 사용량 90% 이상 감소
5. **실시간 연동**: WebSocket/API 하이브리드 모드 정상 동작
6. **타임프레임**: 1M 포함 모든 타임프레임 지원
7. **호가창**: 실시간 호가 데이터 부드러운 업데이트
8. **성능**: 60fps 이상 부드러운 차트 업데이트 유지
