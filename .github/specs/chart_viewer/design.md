...
```
┌──────────┬──────────────────────────────────────────────────┬─────────────┐
│  Coin    │                  Chart Area (4)                  │  Orderbook  │
│  List    │  ┌────────────────────────────────────────────┐  │             │
│          │  │         Main Plot (Candlestick)            │  │   실시간    │
│ (1 비율) │  │              (3 비율)                      │  │   호가창    │
│          │  ├────────────────────────────────────────────┤  │             │
│          │  │      Sub Plot (Volume/MACD/RSI)            │  │  (2 비율)   │
│          │  │              (1 비율)                      │  │             │
│          │  └────────────────────────────────────────────┘  │             │
└──────────┴──────────────────────────────────────────────────┴─────────────┘
```
...
│   ├── chart_area_view.py            # 차트 영역 (PyQtGraph)
│   ├── orderbook_view.py             # 호가창 (실시간)
│   └── control_panel_view.py         # 타임프레임/소스 컨트롤
├── presenters/
│   ├── window_lifecycle_presenter.py # 창 상태 기반 리소스 관리
│   ├── chart_presenter.py            # 차트 로직 (이벤트 기반)
...
└── widgets/
    ...
    ├── subplot_widget.py             # 서브플롯 (Volume/MACD/RSI)
    └── orderbook_widget.py           # 호가 바차트
```
