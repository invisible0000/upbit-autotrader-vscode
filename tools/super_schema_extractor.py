...
1. python super_schema_extractor.py                    # settings DB 스키마 추출 (기본)
2. python super_schema_extractor.py settings          # settings DB 스키마 추출
3. python super_schema_extractor.py market_data       # market_data DB 스키마 추출
4. python super_schema_extractor.py strategies        # strategies DB 스키마 추출
    ...
    def _extract_schema_content(self, db_path: str, db_name: str) -> tuple[str, int]:
        ...
        with sqlite3.connect(db_path) as conn:
            ...
            schema_lines = [
                ...
                f"-- 생성일: {timestamp}",
                f"-- 추출 도구: Super Schema Extractor",
...
def print_usage():
    ...
    """
    print("""
🔄 Super Schema Extractor 사용법:

🤖 === LLM을 위한 핵심 가이드 ===
가장 많이 사용할 명령어들:

1️⃣ GUI 스키마 파일 업데이트 (가장 기본, 자주 사용):
   python super_schema_extractor.py

2️⃣ 현재 상태 백업 생성:
   python super_schema_extractor.py backup

3️⃣ 사용 가능한 DB 확인:
   python super_schema_extractor.py list

📋 === 전체 명령어 목록 ===
기본 사용:
  python super_schema_extractor.py              # settings DB → GUI 스키마 파일 업데이트 ⭐ 가장 유용
  python super_schema_extractor.py settings     # settings DB 스키마 추출
  python super_schema_extractor.py market_data  # market_data DB 스키마 추출
  python super_schema_extractor.py strategies   # strategies DB 스키마 추출

백업 및 관리:
  python super_schema_extractor.py backup       # 타임스탬프 백업 파일 생성 ⭐ 중요한 변경 전 필수
  python super_schema_extractor.py list         # 사용 가능한 DB 목록 표시
