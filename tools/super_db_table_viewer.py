...
class SuperDBTableViewer:
    ...
    def search_records(self, table_name: str, search_term: str, db_name: str = 'settings') -> None:
        """
        🤖 LLM 추천: 특정 값으로 레코드 검색 - 문제 진단 시 사용
        테이블에서 특정 검색어가 포함된 레코드 조회

        사용 예시:
        viewer.search_records('tv_trading_variables', 'RSI')     # RSI 포함된 변수 검색
        viewer.search_records('tv_trading_variables', '수정됨')   # '수정됨' 포함된 레코드 검색
        viewer.search_records('tv_variable_parameters', 'RSI')   # RSI 관련 파라미터 검색

        출력 예시:
        🔍 === tv_trading_variables 테이블에서 'RSI' 검색 ===
        📋 검색 결과: 2개 레코드 발견
        📄 레코드 1: variable_id=RSI, display_name_ko=RSI 지표...
        """
        if db_name not in self.db_paths:
            print(f"❌ 알 수 없는 DB: {db_name}")
            return
        ...
        try:
            with sqlite3.connect(db_path) as conn:
                ...
                print(f"🔍 === {table_name} 테이블에서 '{search_term}' 검색 ===")
                ...
                print(f"📋 검색 결과: {len(results)}개 레코드 발견")

                if results:
                    print(f"📊 검색된 컬럼: {', '.join(text_columns)}")
                    print()

                    for i, record in enumerate(results, 1):
                        print(f"📄 레코드 {i}:")
                        for j, (col_name, value) in enumerate(zip(col_names, record)):
                            # 검색어가 포함된 값은 하이라이트
...
def main():
    ...
    - 'search 테이블명 검색어': 특정 테이블에서 검색어 포함된 레코드 조회
    ...
    4. python super_db_table_viewer.py search tv_trading_variables RSI  # RSI 검색
    """
    import sys
    ...
    if len(sys.argv) == 1:
        ...
        cmd, table_name, search_term = sys.argv[1], sys.argv[2], sys.argv[3]
        if cmd == "search":
            viewer.search_records(table_name, search_term)
        else:
            print(f"❌ 알 수 없는 명령어: {cmd}")
            print_usage()
    else:
...
4️⃣ 문제 진단용 검색 (신규 추가):
   python super_db_table_viewer.py search tv_trading_variables RSI
   python super_db_table_viewer.py search tv_trading_variables "수정됨"
...
검색 기능 (신규):
  python super_db_table_viewer.py search 테이블명 검색어    # 특정 값 검색
  python super_db_table_viewer.py search tv_trading_variables RSI
  python super_db_table_viewer.py search tv_variable_parameters "수정됨"

💡 === 활용 시나리오 ===
• DB 문제 진단: 전체 요약으로 시작 → 검색으로 문제 레코드 찾기 → 특정 테이블 상세 조회
• 마이그레이션 검증: compare로 스키마 차이점 확인
• 데이터 분석: 테이블 구조로 컬럼 정보와 샘플 데이터 확인
• 변수 시스템 디버깅: tv_* 테이블들의 레코드 수와 상태 확인
• 텍스트 검색: 특정 문자열이 포함된 레코드 찾기 (예: "수정됨", "RSI" 등)
""")
