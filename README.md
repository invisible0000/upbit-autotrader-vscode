...
## 🎯 **NEW! 원자적 전략 빌더 (Phase 1 완료)** ✅

> **"누구나 5분 내에 강력한 매매 전략을 만들고, 즉시 백테스트하여 검증할 수 있는 올인원 플랫폼"**

### 🧩 **Variable → Condition → Action → Rule → Strategy**
- **📊 Variable**: RSI, 가격, 볼륨 등 원자적 데이터 소스
- **🎯 Condition**: Variable에 대한 논리적 조건 (`rsi < 20`)  
- **⚡ Action**: 조건 만족 시 실행할 구체적 행동 (`market_buy 5%`)
- **🔧 Rule**: Condition + Action 조합 ("RSI < 20이면 5% 매수")
- **🏆 Strategy**: 완전한 매매 전략 (여러 Rule의 집합)
...
```bash
# 전략 빌더 UI 실행
python ui_prototypes/atomic_strategy_builder_ui.py

# 7규칙 전략 예제 확인  
python ui_prototypes/seven_rule_strategy_example.py
```
