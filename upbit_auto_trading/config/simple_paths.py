...
class SimplePaths:
    ...
    def __init__(self):
        # 설치 루트 자동 감지 (현재 파일 기준 3단계 상위)
        self.APP_ROOT = Path(__file__).parent.parent.parent
        
        # 루트 레벨 디렉토리 구조 (설치형/포터블 최적화)
        self.DATA_DIR = self.APP_ROOT / "data"           # 루트/data (일반 데이터)
        ...
        # 보안 디렉토리 (API 키 등 민감정보)
        self.SECURE_DIR = self.CONFIG_DIR / "secure"     # config/secure (API 키 전용)
    ...
    def _ensure_directories(self):
        ...
        directories = [
            ...
            self.LOGS_DIR,
            self.BACKUPS_DIR,
            self.SECURE_DIR  # API 키 보안 폴더
        ]
        for directory in directories:
            directory.mkdir(parents=True, exist_ok=True)

        # 보안 폴더 권한 설정 (Windows에서는 제한적)
        if self.SECURE_DIR.exists():
            try:
                # 보안 폴더에 더 엄격한 권한 설정 시도
                import stat
                self.SECURE_DIR.chmod(stat.S_IRWXU)  # 소유자만 읽기/쓰기/실행
            except (ImportError, OSError):
                pass  # Windows에서는 chmod가 제한적이므로 무시
