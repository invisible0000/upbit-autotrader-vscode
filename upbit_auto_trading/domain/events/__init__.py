...
from .domain_event_publisher import (
    ...
    subscribe_to_all_domain_events,
...
# 편의 함수들 (하위 호환성 유지)
def subscribe_to_domain_event(event_type: str, handler: Callable[[DomainEvent], None]) -> None:
    """
    도메인 이벤트 구독 편의 함수

    Args:
        event_type: 구독할 이벤트 타입 (문자열)
        handler: 이벤트 처리 함수
    """
    subscribe_to_domain_events(event_type, handler)
