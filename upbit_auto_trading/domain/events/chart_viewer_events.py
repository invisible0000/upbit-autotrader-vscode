"""
차트뷰어 관련 도메인 이벤트 정의 (SQLModel 패턴 적용)

기존 DomainEvent를 상속하여 차트뷰어 전용 이벤트 타입들을 정의.
기존 시스템과 완전 격리되며, 호환성을 보장합니다.

SQLModel 패턴을 참고하여 dataclass 필드 순서 문제를 해결했습니다:
- 모든 필드에 기본값 제공
- field() 없이 직접 기본값 할당
- 상속 계층에서 필드 순서 문제 방지
"""
...
class ChartViewerEvent(DomainEvent):
    ...
    SQLModel 패턴에서 HeroBase와 같은 역할.
    모든 필드에 기본값을 제공하여 상속 시 필드 순서 문제를 방지합니다.
    """
    chart_id: str = ""
    symbol: str = ""
    data_type: str = ""  # "candle", "orderbook", "indicator", "lifecycle"
    timeframe: str = "1m"  # "1m"~"1M" (1개월 포함)
...
class CandleDataEvent(ChartViewerEvent):
    """캔들 데이터 업데이트 이벤트"""
    candle_data: Dict[str, Any] = field(default_factory=dict)  # OHLCV 데이터
    ...
    def __post_init__(self):
        ...
        # priority_level 기본값 설정 (차트뷰어 우선순위)
        if not hasattr(self, 'priority_level') or self.priority_level == 5:
            object.__setattr__(self, 'priority_level', 5)  # ChartViewerPriority.CHART_HIGH


@dataclass(frozen=True)
class OrderbookDataEvent(ChartViewerEvent):
    """호가창 데이터 업데이트 이벤트"""
    orderbook_data: Dict[str, Any] = field(default_factory=dict)  # 매수/매도 호가 데이터
    ...
    def __post_init__(self):
        ...
        # priority_level 기본값 설정 (차트뷰어 우선순위)
        if not hasattr(self, 'priority_level') or self.priority_level == 5:
            object.__setattr__(self, 'priority_level', 8)  # ChartViewerPriority.CHART_BACKGROUND
        # timeframe은 호가창에서 사용하지 않으므로 기본값 설정
        if not self.timeframe or self.timeframe == "1m":
...
class TechnicalIndicatorEvent(ChartViewerEvent):
    ...
    def __post_init__(self):
        ...
        # priority_level 기본값 설정 (차트뷰어 우선순위)
        if not hasattr(self, 'priority_level') or self.priority_level == 5:
            object.__setattr__(self, 'priority_level', 10)  # ChartViewerPriority.CHART_LOW
...
class ChartLifecycleEvent(ChartViewerEvent):
    ...
    resource_priority: int = 5  # 5(활성), 8(비활성), 10(최소화)
