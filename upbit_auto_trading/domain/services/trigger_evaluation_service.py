...
Domain Event 발행을 통한 평가 결과 알림 지원
"""
...
from upbit_auto_trading.domain.events.trigger_events import (
    TriggerEvaluatedEvent, TriggerActivatedEvent, TriggerEvaluationFailedEvent
)
...
class TriggerEvaluationService:
    ...
    def evaluate_trigger(self, trigger: Trigger, market_data: MarketData) -> EvaluationResult:
        ...
        try:
            ...
            # 6. 평가 결과 이벤트 발행
            evaluation_event = TriggerEvaluatedEvent(
                trigger_id=str(trigger.trigger_id),
                strategy_id="UNKNOWN",  # 트리거에서 strategy_id 추출 필요 시 수정
                symbol=market_data.symbol,
                evaluation_result=comparison_result,
                evaluation_timestamp=market_data.timestamp,
                market_data={
                    "current_value": current_value,
                    "target_value": target_value,
                    "operator": trigger.operator.value,
            ...
            # 7. 트리거 활성화 이벤트 발행 (조건 만족 시)
            if comparison_result:
                activation_event = TriggerActivatedEvent(
                    trigger_id=str(trigger.trigger_id),
                    strategy_id="UNKNOWN",  # 트리거에서 strategy_id 추출 필요 시 수정
                    symbol=market_data.symbol,
                    activation_timestamp=market_data.timestamp,
                    activation_conditions=[
                        f"{trigger.variable.display_name} {trigger.operator.value} {target_value} (현재값: {current_value})"
                    ]
                )
            ...
            # 평가 실패 이벤트 발행
            failure_event = TriggerEvaluationFailedEvent(
                trigger_id=str(trigger.trigger_id),
                strategy_id="UNKNOWN",  # 트리거에서 strategy_id 추출 필요 시 수정
                symbol=market_data.symbol,
                error_message=str(e),
