...
class GlobalWebSocketManager:
    ...
    def __init__(self):
        ...
        # 물리적 WebSocket 연결 (네이티브 v6 클라이언트)
    ...
    async def initialize_public_connection(
        self,
        client: Optional[NativeWebSocketClient] = None
    ) -> bool:
        ...
        try:
            ...
            # v6 클라이언트의 메시지 핸들러를 v6 라우터로 연결
            await self._setup_v6_message_handler(client, WebSocketType.PUBLIC)
            ...
            return False    async def initialize_private_connection(
        self,
    ...
    async def _setup_v6_message_handler(
        self,
        client: NativeWebSocketClient,
        connection_type: WebSocketType
    ) -> None:
        """v6 네이티브 클라이언트 메시지 핸들러 설정"""
        try:
            async def v6_message_handler(raw_message: Dict[str, Any]) -> None:
                try:
                    # v6 네이티브 이벤트 변환
                    v6_event = await self._convert_message_to_v6_event(
                    ...
                    self.logger.error(f"v6 메시지 처리 실패: {e}")
                    metrics = self._connection_metrics[connection_type]
                    metrics.error_count += 1

            # v6 클라이언트에 핸들러 설정
            client.message_handler = v6_message_handler

            self.logger.info(f"{connection_type} v6 메시지 핸들러 설정 완료")

        except Exception as e:
            self.logger.error(f"{connection_type} 핸들러 설정 실패: {e}")
            raise ConnectionError(f"핸들러 설정 오류: {e}")

    async def _convert_message_to_v6_event(
        self,
