...
class ChunkProcessor:
    ...
    def _log_chunk_info_debug(
        ...
        try:
            ...
            overlap_status = getattr(chunk_info, 'overlap_status', None)

            # datetime 속성들을 안전하게 문자열로 변환
            final_candle_start = getattr(chunk_info, 'final_candle_start', None)
            final_candle_end = getattr(chunk_info, 'final_candle_end', None)
            ...
            debug_data = {
                ...
                "final_candle_start": (
                    final_candle_start.isoformat()
                    if hasattr(final_candle_start, 'isoformat')
                    else str(final_candle_start) if final_candle_start else None
                ),
                "final_candle_end": (
                    final_candle_end.isoformat()
                    if hasattr(final_candle_end, 'isoformat')
                    else str(final_candle_end) if final_candle_end else None
                ),
                ...
                # 겹침 상태
                "overlap_status": overlap_status.value if overlap_status else None,
    ...
    async def _process_current_chunk(
        ...
        try:
            ...
            # 🔍 청크 처리 완료 로그
            self._log_chunk_info_debug(chunk_info, status="completed", processing_time_ms=100.0)
    ...
    def _update_collection_state(
        ...
        if chunk_result.success and collection_state.current_chunk:
            ...
            # 🔍 완료된 청크 로그
            self._log_chunk_info_debug(
                collection_state.current_chunk,
                status="added_to_completed",
    ...
    def _prepare_next_chunk(self, collection_state: InternalCollectionState) -> None:
        ...
        # 🔍 다음 청크 준비 로그
        self._log_chunk_info_debug(next_chunk, status="prepared_next")
