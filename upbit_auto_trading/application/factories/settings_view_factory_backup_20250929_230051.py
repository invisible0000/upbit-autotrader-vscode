...
class BaseComponentFactory(ABC):
    ...
    def _ensure_application_context(self):
        ...
        if not context or not hasattr(context, 'is_initialized') or not context.is_initialized:
            self._logger.warning("⚠️ ApplicationContext 초기화 상태 불확실 - 계속 진행")
        return context

    def _get_application_container(self):
        """표준 ApplicationServiceContainer 접근 - 모든 하위 Factory에서 사용"""
        # 1. ApplicationContext 상태 확인 (경고만, 블록하지 않음)
        try:
            self._ensure_application_context()
        except Exception as e:
            self._logger.warning(f"⚠️ ApplicationContext 확인 실패: {e} - Container 직접 접근")

        # 2. ApplicationServiceContainer 접근
        ...
        if app_container is None:
            error_msg = f"❌ ApplicationServiceContainer 미초기화 - {self.__class__.__name__}"
            self._logger.error(error_msg)
            raise RuntimeError(error_msg)

        self._logger.debug(f"✅ ApplicationServiceContainer 접근 성공 - {self.__class__.__name__}")
        return app_container

    def _get_service_safely(self, service_getter, service_name: str):
        """서비스 안전 접근 패턴 - 표준 오류 처리"""
        try:
            service = service_getter()
            if service is None:
                raise RuntimeError(f"{service_name} 서비스가 None")
            self._logger.debug(f"✅ {service_name} 서비스 접근 성공")
            return service
        except AttributeError as e:
            error_msg = f"❌ {service_name} 서비스 메서드 미구현: {e}"
            self._logger.error(error_msg)
            raise RuntimeError(error_msg) from e
        except Exception as e:
            error_msg = f"❌ {service_name} 서비스 접근 실패: {e}"
            self._logger.error(error_msg)
            raise RuntimeError(error_msg) from e
...
class ApiSettingsComponentFactory(BaseComponentFactory):
    ...
    def create_component_instance(self, parent: Optional[QWidget], **kwargs) -> QWidget:
        ...
        # 1. 서비스 접근 (생성자 주입 우선, 표준 Container 접근 대체)
        if self._api_key_service is not None:
            ...
            self._logger.info("✅ 생성자 주입 서비스 사용")
        else:
            # 표준 ApplicationServiceContainer 접근 사용
            container = self._get_application_container()
            
            api_key_service = self._get_service_safely(
                lambda: container.get_api_key_service(),
                "ApiKeyService"
            )
            app_logging_service = self._get_service_safely(
                lambda: container.get_logging_service(),
                "LoggingService"
            )
