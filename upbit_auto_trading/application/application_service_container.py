"""
Application Service Container - Application Layer 서비스 조합 전담

Application Layer Services Orchestration:
- Use Case Services (Strategy, Trigger, Backtest)
- Application Services (Settings, Logging, Lifecycle)
- Event Integration (Domain Event ↔ Application Handler)
- Infrastructure Service Integration (External Dependency Container 경유)

Clean Architecture Application Layer의 모든 비즈니스 로직 조합을
중앙 집중식으로 관리하여 Presentation Layer에 깨끗한 인터페이스를 제공합니다.
"""
...
class ApplicationServiceContainer:
    ...
    def get_api_key_service(self) -> 'ApiKeyService':
        ...
        if "api_key_service" not in self._services:
            # External Dependency Container에서 ApiKeyService 가져오기
    ...
    def get_database_service(self) -> 'DatabaseConnectionService':
        ...
        if "database_service" not in self._services:
            # External Dependency Container에서 DatabaseConnectionService 가져오기
            from upbit_auto_trading.infrastructure.dependency_injection import get_external_dependency_container

            external_container = get_external_dependency_container()
            self._services["database_service"] = external_container.database_manager()
    ...
    def get_settings_service(self) -> 'SettingsService':
        ...
        if "settings_service" not in self._services:
            # External Dependency Container에서 SettingsService 가져오기
            from upbit_auto_trading.infrastructure.dependency_injection import get_external_dependency_container

            external_container = get_external_dependency_container()
            self._services["settings_service"] = external_container.settings_service()
    ...
    def get_profile_service(self):
        ...
        Returns:
            ProfileMetadataService: 프로필 메타데이터 관리 서비스
        """
