        ...
        try:
            ...
            try:
                ...
                if existing_strategy:
                    # 기존 전략 업데이트
                    previous_params = json.loads(existing_strategy.parameters) if existing_strategy.parameters else {}
                    ...
                    # 도메인 이벤트 발행: 전략 업데이트
                    strategy_updated_event = StrategyUpdated(
                        ...
                        previous_version=existing_strategy.updated_at.isoformat() if existing_strategy.updated_at else None
                    )
                    publish_domain_event(strategy_updated_event)
                    ...
                    # 변경사항 커밋
                    session.commit()
                    self.logger.info(f"전략 '{strategy_id}'이(가) 생성되었습니다.")

                    # 도메인 이벤트 발행: 전략 생성
                    strategy_created_event = StrategyCreated(
                        strategy_id=strategy_id,
                        strategy_name=name,
                    ...
                    publish_domain_event(strategy_created_event)
                return True
