...
class ConditionBuilderWidget(QWidget):
    ...
    def _setup_widget_connections(self):
        ...
        try:
            ...
            # 비교 설정 → 미리보기 업데이트
            self.comparison_settings_widget.settings_changed.connect(
    ...
    def _on_variable_changed(self, var_id: str, var_info: Dict[str, Any]):
        ...
        try:
            ...
            if var_info.get('category') == 'external':
                if hasattr(self.conditional_compatibility_widget, 'show'):
                    self.conditional_compatibility_widget.show()
                # 호환성 체크 실행
                self._check_compatibility()
            else:
                if hasattr(self.conditional_compatibility_widget, 'hide'):
                    self.conditional_compatibility_widget.hide()
    ...
    def _check_compatibility(self):
        ...
        try:
            if self.conditional_compatibility_widget:
                ...
                if external_var_id and hasattr(self.conditional_compatibility_widget, 'check_compatibility'):
                    # 호환성 체크
    ...
    def _update_preview(self):
        ...
        try:
            if self.condition_preview_widget:
                ...
                # 미리보기 업데이트
                if hasattr(self.condition_preview_widget, 'update_preview'):
    ...
    def _on_create_clicked(self):
        ...
        try:
            ...
            if condition_data.get('category') == 'external':
                if self.conditional_compatibility_widget and hasattr(self.conditional_compatibility_widget, 'check_compatibility'):
                    try:
                        external_var_id = condition_data.get('variable')
                        if external_var_id:
                            is_compatible = self.conditional_compatibility_widget.check_compatibility(external_var_id)
                        ...
                        self.logger.warning(f"호환성 체크 실패: {compat_error}")
    ...
    def load_condition(self, condition_data: Dict[str, Any]):
        ...
        try:
            ...
            # 위젯들에 데이터 로드 (메서드가 있는 경우만)
            if self.variable_selection_widget:
                # 기본적인 선택 복원 시도
                variable_id = condition_data.get('variable')
                if variable_id:
                    self.logger.info(f"변수 '{variable_id}' 로드 시도")

            if self.comparison_settings_widget:
                # 기본적인 설정 복원 시도
                self.logger.info("비교 설정 로드 시도")
    ...
    def clear_all_inputs(self):
        ...
        try:
            ...
            # 위젯들 초기화 (메서드가 있는 경우만)
            if self.variable_selection_widget:
                self.logger.info("변수 선택 위젯 초기화")

            if self.comparison_settings_widget:
                self.logger.info("비교 설정 위젯 초기화")
            ...
            if self.condition_preview_widget:
                if hasattr(self.condition_preview_widget, 'clear'):
                    self.condition_preview_widget.clear()
