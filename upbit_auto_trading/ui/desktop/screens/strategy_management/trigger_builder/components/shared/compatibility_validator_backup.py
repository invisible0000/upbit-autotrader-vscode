...
# -*- coding: utf-8 -*-
"""
통합 호환성 검증기 (CompatibilityValidator) - 변수 간 의미론적 호환성 검증
===========================================================================

역할: 변수 간 비교가 논리적으로 의미있는지 검증
- 변수 타입 분류 (가격, 오실레이터, 모멘텀, 거래량, 변동성, 재무)
- 척도 타입 분류 (가격 단위, 퍼센트, 거래량, 비율, 정규화)
- 호환성 규칙 (같은 타입과 척도끼리만 비교 허용)
- 대안 제안 및 신뢰도 점수 계산

호환성 예시:
- ✅ RSI ↔ 스토캐스틱 (같은 오실레이터, 0-100 퍼센트 척도)
- ✅ 현재가 ↔ 이동평균 (같은 가격 척도)
- ❌ RSI ↔ MACD (다른 타입과 척도)
- ❌ 현재가 ↔ 거래량 (완전히 다른 의미와 단위)

조건 검증기(ConditionValidator)와의 차이점:
- CompatibilityValidator: 변수 조합의 의미론적 타당성 (변수 선택 시)
- ConditionValidator: 입력값의 형식과 범위 검증 (조건 저장 시)

검증 시점: 변수 선택 및 조합 시 실시간 호환성 체크

통합 기능:
- 기본 호환성 검증 (레거시 카테고리 기반)
- 고급 타입 기반 검증 (VariableType, ScaleType)
- 복합 조건 검증 (다중 변수 호환성)
- 대안 제안 (호환 가능한 변수 추천)
- 신뢰도 점수 계산 (호환성 정도 정량화)
"""
