"""
데이터 소스 선택 UI 위젯 - 간단한 미니 시뮬레이션 버전
기존 strategy_management_backup 구조를 따라 단순화됨
"""
...
logger = create_component_logger("SimpleDataSourceSelector")


class SimpleDataSourceSelectorWidget(QWidget):
    """
    간단한 데이터 소스 선택 위젯
    4개 선택지: 내장 최적화, 실제 DB, 합성 현실적, 단순 폴백
    향후 안정화되면 3개 또는 1-2개로 통일 예정
    """
    ...
    def __init__(self, parent=None):
        ...
        self.manager = None
        logger.debug("SimpleDataSourceSelectorWidget 초기화 시작")
        self.init_ui()
...
def create_simple_data_source_selector(parent=None) -> SimpleDataSourceSelectorWidget:
    """
    간단한 데이터 소스 선택기 팩토리 함수
    기존 복잡한 DDD 구조 대신 단순한 위젯만 반환
    """
    logger.debug("SimpleDataSourceSelectorWidget 생성")
    return SimpleDataSourceSelectorWidget(parent)
