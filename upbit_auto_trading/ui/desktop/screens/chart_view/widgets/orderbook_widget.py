...
class OrderbookWidget(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None, event_bus: Optional[InMemoryEventBus] = None):
        ...
        # WebSocket 우선 + REST 백업 타이머 (실거래 환경)
        self._websocket_primary_timer = QTimer(self)
        self._websocket_primary_timer.timeout.connect(self._check_websocket_status)
        self._websocket_primary_timer.start(1000)  # 1초마다 웹소켓 상태 체크

        # REST API 백업 타이머 (웹소켓 실패시에만 활성화)
        self._rest_backup_timer = QTimer(self)
        self._rest_backup_timer.timeout.connect(self._refresh_orderbook)
        # 시작하지 않음 - 웹소켓 실패시에만 활성화

        # 실시간 호가 업데이트 타이머 제거 (웹소켓 우선으로 변경)
        # self._refresh_timer = QTimer(self)
        # self._refresh_timer.timeout.connect(self._refresh_orderbook)
        # self._refresh_timer.start(100)  # 0.1초마다 자동 갱신 (테스트용)
    ...
    def _setup_ui(self) -> None:
        ...
        self._websocket_status_label.setStyleSheet("color: #888; font-size: 10px;")
        ...
        self._price_info_label.setStyleSheet("color: #666; font-size: 10px;")
        ...
        self._market_info_label.setStyleSheet("color: #888; font-size: 9px;")
        ...
        click_info.setStyleSheet("color: #666; font-size: 8px;")
    ...
    def _check_websocket_status(self) -> None:
        ...
        if not self._websocket_initialized or not self._websocket_service:
            # 웹소켓 미초기화시 REST 백업 활성화
            if not self._rest_backup_timer.isActive():
                self._rest_backup_timer.start(500)  # 0.5초 간격으로 REST 백업
                self._logger.info("🔄 웹소켓 미준비, REST API 백업 모드 활성화")
                if self._websocket_status_label:
                    self._websocket_status_label.setText("🟡 REST 백업 모드")
        else:
            # 웹소켓 정상시 REST 백업 비활성화
            if self._rest_backup_timer.isActive():
                self._rest_backup_timer.stop()
                self._logger.info("✅ 웹소켓 복구, REST 백업 모드 비활성화")
                if self._websocket_status_label:
                    self._websocket_status_label.setText("🟢 웹소켓 우선 모드")
