...
class CoinListWidget(QWidget):
    ...
    favorite_toggled = pyqtSignal(str, bool)  # 즐겨찾기 토글 시그널
    ...
    def _setup_ui(self) -> None:
        ...
        # UI 구성 상태 확인
        if self._coin_list:
            self._logger.debug("UI 구성 완료 - _coin_list 정상 생성됨")
        else:
            self._logger.error("UI 구성 오류 - _coin_list가 None임")
    ...
    def _update_ui_simple(self) -> None:
        """UI 업데이트 - 매우 단순"""
        # _coin_list가 None이면 재생성 시도 (중복 방지)
        if not self._coin_list:
            self._logger.warning("⚠️ _coin_list가 None입니다. 재생성 시도...")
            try:
                from PyQt6.QtWidgets import QListWidget
                self._coin_list = QListWidget()
                self._coin_list.itemClicked.connect(self._on_item_clicked)

                # 기존 레이아웃에서 마지막 위젯 제거 후 새로 추가
                layout = self.layout()
                if layout:
                    # 기존에 QListWidget이 있다면 제거
                    for i in range(layout.count()):
                        item = layout.itemAt(i)
                        if item and item.widget() and isinstance(item.widget(), QListWidget):
                ...
                self._logger.info("✅ _coin_list 재생성 및 레이아웃 정리 완료")
            except Exception as e:
                self._logger.error(f"❌ _coin_list 재생성 실패: {e}")
                return
