...
- [ ] **1.1 DB 스키마 분석**
  - [ ] 모든 tv_ 테이블 목록 완전 파악 (tv_trading_variables, tv_variable_parameters, tv_* 전체)
  - [ ] 각 tv_ 테이블 구조 및 데이터 상태 상세 확인
  - [ ] 테이블 간 관계 및 외래키 분석
  - [ ] 누락된 변수 및 파라미터 식별
  - [ ] 데이터 품질 문제 파악

- [ ] **1.2 DB-YAML 매핑 분석**
  - [ ] DB 변수 vs YAML 변수 완전 매핑 테이블 생성
  - [ ] 누락/불일치/중복 항목 정확한 분류
  - [ ] 각 변수별 데이터 품질 비교 (DB vs YAML)

- [ ] **1.3 메타변수 특별 분석**
  - [ ] META_PYRAMID_TARGET, META_TRAILING_STOP DB 등록 상태 확인
  - [ ] 메타변수 파라미터 정의 상태 검증
  - [ ] 트리거 빌더 연동 실패 원인 정확한 진단
