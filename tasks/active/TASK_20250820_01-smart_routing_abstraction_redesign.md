...
- [ ] 1.2 도메인 모델 정의 (TradingSymbol, Timeframe, 데이터 타입들)
- [ ] 1.3 시간 범위 요청 파라미터 명확화 (start_time, end_time, count 우선순위)
- [ ] 1.4 기본 예외 처리 체계 설계

### Phase 2: 최소 동작 가능한 구현
- [ ] 2.1 SmartDataRouter 기본 구현 (기존 인터페이스 유지)
- [ ] 2.2 UpbitProvider REST 구현 (최소 캔들 데이터)
- [ ] 2.3 Request/Response 모델 변환 로직
- [ ] 2.4 기본 채널 선택 로직 (REST 우선)
