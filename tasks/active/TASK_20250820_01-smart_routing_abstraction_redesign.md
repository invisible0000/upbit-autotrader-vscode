...
- [-] 3.1 FrequencyAnalyzer 구현 (요청 패턴 분석) ❌ **재작업 필요**
  - ⚠️ **계산 로직 오류 발견**: 실제 요청 시간 범위 무시, 고정 5분 윈도우로 잘못된 분당 요청수 계산
  - ⚠️ **WebSocket 권장 로직 오작동**: 30초간 초당1회(60분당) → 6분당으로 오계산 → WebSocket 권장하지 않음
  - ⚠️ **테스트 코드 부정확**: 구현 오류에 맞춰 테스트 수정, 올바른 동작 검증 실패
  - 🔄 **재작업 필요**: 동적 윈도우 계산, 실제 요청 시간 범위 기반 분당 요청수 계산
  - 🧹 **정리 완료**: 잘못된 테스트 파일들 삭제됨
