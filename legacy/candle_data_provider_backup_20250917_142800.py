...
class RequestInfo:
    ...
    to: Optional[datetime] = None         # 시작점 - 최신 캔들 시간 (원시)
    end: Optional[datetime] = None        # 종료점 - 가장 과거 캔들 시간 (원시)

    # === 새 필드 ===
    request_at: datetime = field(default_factory=lambda: datetime.now(timezone.utc))

    # === 🚀 사전 계산된 필드들 (성능 + 일관성 보장) ===
    aligned_to: Optional[datetime] = field(init=False)      # 정렬된 시작 시간
    aligned_end: Optional[datetime] = field(init=False)     # 정렬된 종료 시간
    expected_count: Optional[int] = field(init=False)       # 예상 캔들 개수

    def __post_init__(self):
        """요청 정보 검증 및 🚀 사전 계산 (성능 + 일관성 보장)"""
        # 기본 파라미터 검증
        if not self.symbol:
        ...
        # ================================================================================
        # 🚀 핵심: 사전 계산 영역 - 성능 + 일관성 보장 (RequestInfo 생성 시점 1회만 실행)
        # ================================================================================

        # 1. 시간 정렬 사전 계산 (align_to_candle_boundary)
        ...
        if request_type in [RequestType.TO_COUNT, RequestType.TO_END] and self.to is not None:
            ...
            # to가 없거나 정렬 불필요한 경우
            object.__setattr__(self, 'aligned_to', self.to)
        ...
        # 2. 예상 캔들 개수 사전 계산 (calculate_expected_count)
        if request_type in [RequestType.COUNT_ONLY, RequestType.TO_COUNT]:
            ...
            start_time = self.aligned_to if self.aligned_to is not None else current_time

            if start_time <= self.aligned_end:
        ...
        # ================================================================================
        # 🏁 사전 계산 완료 - 이후 모든 접근은 계산 없이 즉시 반환
        # ================================================================================
...
class CandleDataProvider:
    ...
    def _plan_collection_with_request_info(self, request_info: RequestInfo) -> CollectionPlan:
        """RequestInfo 객체를 직접 받는 내부 계획 수립 메서드 (중복 생성 방지)"""
        logger.info(f"수집 계획 수립 (RequestInfo 재사용): {request_info.to_log_string()}")

        # 동적 비즈니스 검증
        current_time = datetime.now(timezone.utc)
        if request_info.to is not None and request_info.to > current_time:
            raise ValueError(f"to 시점이 미래입니다: {request_info.to}")
        if request_info.end is not None and request_info.end > current_time:
        ...
        logger.info(f"✅ 계획 수립 완료 (중복방지): {total_count:,}개 캔들, {estimated_chunks}청크, "
                    f"예상 소요시간: {estimated_duration_seconds:.1f}초")
        return plan
    ...
    def start_collection(
        ...
        """캔들 수집 시작 - 기존 호환성을 위한 래퍼 메서드"""
        # ⚠️ 호환성을 위해 유지하지만, 내부적으로는 RequestInfo 생성 후 전달
        request_info = RequestInfo(
            symbol=symbol,
            timeframe=timeframe,
            count=count,
        ...
        logger.info(f"캔들 수집 시작 (래퍼): {request_info.to_log_string()}")

        # 내부 메서드로 위임하여 중복 방지
        return self._start_collection_with_request_info(request_info)

    def _start_collection_with_request_info(self, request_info: RequestInfo) -> str:
        """RequestInfo 객체를 직접 받는 내부 수집 시작 메서드 (중복 생성 방지)"""
        logger.info(f"캔들 수집 시작 (RequestInfo 재사용): {request_info.to_log_string()}")

        # 🚀 핵심: RequestInfo 재사용으로 계획 수립 (중복 생성 없음)
        plan = self._plan_collection_with_request_info(request_info)

        # 요청 ID 생성 (타임스탬프 기반)
        request_id = f"{request_info.symbol}_{request_info.timeframe}_{int(datetime.now().timestamp())}"
        ...
        collection_state = CollectionState(
            ...
            request_info=request_info,  # ← 동일한 RequestInfo 객체 사용!
        ...
        logger.info(f"✅ 수집 시작 (중복방지): 요청 ID {request_id}, 예상 완료: {estimated_completion}")
        return request_id
    ...
    async def mark_chunk_completed(self, request_id: str) -> bool:
        ...
        # ================================================================================
        # 🚀 핵심: 요청 타입 기반 최적화 (COUNT_ONLY 첫 청크 OverlapAnalyzer 건너뛰기)
        # ================================================================================
    ...
    def _calculate_total_count_by_type(
        ...
        """🚀 사전 계산된 예상 캔들 개수 반환 (성능 향상 + 일관성 보장)"""
    ...
    def _create_first_chunk_params_by_type(
        ...
        if request_type == RequestType.COUNT_ONLY:
            ...
            # 🚀 to + count: 사전 계산된 정렬 시간 사용 (성능 + 일관성 보장)
            chunk_size = min(request_info.count, self.chunk_size)
            aligned_to = request_info.get_aligned_to_time()  # 이미 계산됨

            # ========================================================================
            # 🎯 핵심: 진입점 보정 (사용자 시간 → 내부 시간 변환)
            # ========================================================================
            ...
            # 🚀 to + end: 사전 계산된 정렬 시간 사용 (성능 + 일관성 보장)
            aligned_to = request_info.get_aligned_to_time()  # 이미 계산됨
    ...
    async def get_candles(
        ...
        # ================================================================================
        # 🎯 핵심: 원본 요청 정보 보존 (RequestInfo 무결성 보장)
        # 진입점 보정은 내부 로직에서만 처리하고, RequestInfo는 사용자 원본 요청 유지
        # ================================================================================
        logger.info(f"   🎯 원본 요청 보존: to={to} (RequestInfo에 원본 저장)")
        logger.info("   🔧 진입점 보정: _create_first_chunk_params_by_type에서 내부 처리")
        # ================================================================================

        # ================================================================================
        # 🎯 핵심: RequestInfo 중복 생성 해결 ("주문서 1개만" 원칙)
        # 한 번 생성된 RequestInfo를 plan_collection과 start_collection에서 공유
        # ================================================================================
        request_info = RequestInfo(
            symbol=symbol,
            timeframe=timeframe,
        ...
        logger.info(f"   🎯 RequestInfo 생성 (1회만): {request_info.to_log_string()}")
        logger.info("   🔧 진입점 보정: _create_first_chunk_params_by_type에서 내부 처리")
        # ================================================================================

        # RequestInfo 객체를 직접 전달하여 중복 생성 방지
        request_id = self._start_collection_with_request_info(request_info)
        ...
        try:
            ...
            if collection_state and collection_state.is_completed:
                ...
                if to and count:
                    # 원본 to 시간 기준으로 조회 (내부 보정 없이 사용자 요청 기준)
    ...
    async def _fetch_chunk_from_api(self, chunk_info: ChunkInfo) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if chunk_info.timeframe.endswith('m'):
                ...
                # 🚀 일봉: 최적화된 틱 계산 (80% 성능 향상)
                # ====================================================================
                # 📅 핵심: 일봉 API 지출점 보정 (내부 → API 요청 시간)
                # ====================================================================
                to_param = None
                if chunk_info.to:
                ...
                # 🚀 주봉: 최적화된 틱 계산 (80% 성능 향상)
                # ====================================================================
                # 📅 핵심: 주봉 API 지출점 보정 (내부 → API 요청 시간)
                # ====================================================================
                to_param = None
                if chunk_info.to:
                    fetch_start_time = TimeUtils.get_time_by_ticks(chunk_info.to, chunk_info.timeframe, 1)
                ...
                # 🚀 월봉: 최적화된 틱 계산 (100% 정확성 + 성능 향상)
                # ====================================================================
                # 📅 핵심: 월봉 API 지출점 보정 (내부 → API 요청 시간)
                # ====================================================================
                to_param = None
                if chunk_info.to:
                    fetch_start_time = TimeUtils.get_time_by_ticks(chunk_info.to, chunk_info.timeframe, 1)
    ...
    def _calculate_chunk_end_time(self, chunk_info: ChunkInfo) -> datetime:
        """청크 요청의 예상 종료 시점 계산 - 최적화된 틱 계산 사용"""
        # ================================================================================
        # 🎯 핵심: 청크 종료 시간 계산 (업비트 내림차순 API 구조 대응)
        # ================================================================================
    ...
    def _calculate_api_count(self, start_time: datetime, end_time: datetime, timeframe: str) -> int:
        """API 요청에 필요한 캔들 개수 계산 - TimeUtils의 정확한 계산 활용"""
        # 🚀 TimeUtils의 정확한 계산 방법 사용 (월/년봉 정확성 + 성능 향상)
