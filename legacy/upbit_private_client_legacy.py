"""
업비트 프라이빗 API 클라이언트 - 단순화 작업 중 (임시 주석 처리)

인증이 필요한 REST API 기능을 담당합니다.
현재 core/adapters 의존성 제거 작업 중으로 임시 주석 처리됨
"""
...
class UpbitPrivateClient:
    """
    업비트 전용 프라이빗 API 클라이언트 (작업 중)

    주의: 현재 core/adapters 의존성 제거 작업 중으로
    일부 기능이 임시 비활성화되어 있습니다.

    TODO: 단계적으로 업비트 전용 구현으로 교체 예정
    """

    def __init__(self, access_key: Optional[str] = None, secret_key: Optional[str] = None):
        """
        업비트 프라이빗 API 클라이언트 초기화 (임시 단순화)
    ...
    # 임시로 모든 메서드를 주석 처리하고 단계적으로 단순화 예정
