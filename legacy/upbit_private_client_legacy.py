...
class UpbitPrivateClient(BaseApiClient):
    ...
    async def get_order_chance_batch(self, symbols: List[str]) -> Dict[str, Dict[str, Any]]:
        """여러 심볼의 주문 가능 정보 일괄 조회 (단수형 컨벤션)"""
        import asyncio

        if len(symbols) > 10:
            raise ValueError("한 번에 최대 10개 심볼까지만 조회 가능합니다")

        try:
            tasks = [self.get_order_chance(symbol) for symbol in symbols]
