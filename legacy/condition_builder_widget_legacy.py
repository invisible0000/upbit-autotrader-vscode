"""
컨디션 빌더 위젯 - 재사용 가능한 공유 컴포넌트
Legacy condition_dialog.py의 UI 구조를 완전히 복사하여 구현
"""
...
class ConditionBuilderWidget(QWidget):
    ...
    Legacy condition_dialog.py의 UI 레이아웃을 그대로 복사하여 구현
    향후 트리거 빌더, 전략 메이커, 스크리너에서 재사용
    """
    ...
    def _init_ui(self):
        """UI 초기화 - 4개 박스 구조로 깔끔하게 재설계"""
        main_layout = QVBoxLayout(self)
        main_layout.setContentsMargins(5, 5, 5, 5)
        main_layout.setSpacing(8)

        # 1. 변수 선택 (기본 변수)
        self._create_base_variable_area(main_layout)
    ...
    def _create_condition_preview_area(self, parent_layout):
        ...
        # 파라미터 설정 영역 (변수 선택 아래에 배치, 스크롤 가능)
        ...
        self.parameter_scroll.setMaximumHeight(150)  # 최대 높이 제한
