...
⚠️ 레거시 코드입니다. 실제 트리거 빌더는 trigger_builder_screen.py를 사용하세요.
⚠️ 이 파일은 호환성을 위해 유지되지만 새로운 기능은 trigger_builder_screen.py에 구현하세요.
"""
...
try:
    ...
    def get_embedded_simulation_engine():
        return SimulationEngineFallback()
    
    class SimulationEngineFallback:
        def get_scenario_data(self, scenario, length):
            return {'scenario': scenario, 'current_value': 50.0, 'data_source': 'fallback'}
        
        def load_market_data(self, length):
            return None
    
    class TriggerCalculator:
        def calculate_trigger_points(self, data, operator, target):
            return []
    ...
    class SimulationResultWidget(QWidget):
        def __init__(self):
            ...
            layout.addWidget(QLabel("⚠️ 시뮬레이션 결과 위젯을 로드할 수 없습니다."))
        
        def update_simulation_chart(self, **kwargs):
            pass
...
# 레거시 모듈 리로드 함수 (사용하지 않음)
def reload_condition_dialog():
    """조건 다이얼로그 모듈 강제 리로드 - 레거시 함수"""
    print("⚠️ 레거시 리로드 함수는 더 이상 사용되지 않습니다.")
    pass
...
try:
    ...
    from .trigger_builder.components.core.condition_storage import ConditionStorage
    from .components.condition_loader import ConditionLoader  # 올바른 경로: components에 있음
    CORE_COMPONENTS_AVAILABLE = True
    print("✅ 핵심 컴포넌트 로드 성공")
except ImportError as e:
    CORE_COMPONENTS_AVAILABLE = False
    print(f"❌ 핵심 컴포넌트 로드 실패: {e}")
    
    # 폴백 클래스들 정의
    class ConditionDialog(QWidget):
        condition_saved = pyqtSignal(dict)
        edit_mode_changed = pyqtSignal(bool)
        
        def __init__(self):
            super().__init__()
            self.setWindowTitle("조건 다이얼로그 - 로드 실패")
            layout = QVBoxLayout(self)
            layout.addWidget(QLabel("⚠️ 조건 다이얼로그를 로드할 수 없습니다."))
            layout.addWidget(QLabel("실제 기능은 trigger_builder_screen.py를 사용하세요."))
        
        def save_condition(self):
            print("⚠️ 폴백 조건 저장 - 실제 동작하지 않음")
        
        def load_condition(self, condition):
            print("⚠️ 폴백 조건 로드 - 실제 동작하지 않음")
...
class IntegratedConditionManager(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        if SHARED_COMPONENTS_AVAILABLE:
            ...
            print("⚠️ 공유 컴포넌트를 사용할 수 없음 - 폴백 사용")
            self.chart_visualizer = ChartVisualizer()  # 폴백 클래스
            self.simulation_result_widget = SimulationResultWidget()  # 폴백 클래스
            self.simulation_engine = get_embedded_simulation_engine()  # 폴백 함수
            self.trigger_calculator = TriggerCalculator()  # 폴백 클래스
        ...
        # 레거시 경고 메시지
        print("⚠️ IntegratedConditionManager는 레거시 코드입니다.")
        print("⚠️ 실제 트리거 빌더는 trigger_builder_screen.py를 사용하세요.")
    ...
    def create_condition_builder_area(self):
        ...
        if CORE_COMPONENTS_AVAILABLE:
            try:
                ...
                print("✅ 조건 다이얼로그 임베딩 성공")
                
            except Exception as e:
                print(f"❌ 조건 빌더 임베딩 실패: {e}")
                self._create_condition_builder_fallback(layout)
        else:
            print("⚠️ 핵심 컴포넌트 사용 불가 - 폴백 UI 사용")
            self._create_condition_builder_fallback(layout)
        
        return group
    
    def _create_condition_builder_fallback(self, layout):
        """조건 빌더 폴백 UI"""
        # 레거시 경고 메시지
        warning_label = QLabel("⚠️ 조건 빌더 로딩 실패")
        ...
        # 실제 사용 안내
        guide_label = QLabel(
            "실제 트리거 빌더는 trigger_builder_screen.py를 사용하세요.\n"
            "이 파일은 레거시 호환성을 위해서만 유지됩니다."
        )
        ...
        # 빈 조건 다이얼로그 생성 (시그널 호환성 위해)
        self.condition_dialog = ConditionDialog()  # 폴백 클래스 사용
    ...
    def create_mini_chart_widget(self):
        ...
        if SHARED_COMPONENTS_AVAILABLE:
            ...
            layout.addWidget(QLabel("⚠️ 차트 위젯을 로드할 수 없습니다."))
    ...
    def update_chart_with_simulation_results(self, simulation_data, trigger_results,
                                            base_variable_data=None, external_variable_data=None,
                                            variable_info=None):
        """시뮬레이션 결과로 차트 업데이트 - 개선된 SimulationResultWidget 사용"""
        if not SHARED_COMPONENTS_AVAILABLE:
            print("⚠️ 공유 컴포넌트 사용 불가 - 차트 업데이트 건너뜀")
            return
        ...
        try:
            ...
            if hasattr(self.simulation_result_widget, 'update_simulation_chart'):
                ...
                print("⚠️ update_simulation_chart 메서드 없음")
            
        except Exception as e:
            print(f"⚠️ 개선된 차트 업데이트 실패, 폴백 사용: {e}")
            # 폴백: 기존 ChartVisualizer 사용 (있다면)
            if hasattr(self, 'chart_visualizer') and hasattr(self.chart_visualizer, 'update_chart_with_simulation_results'):
