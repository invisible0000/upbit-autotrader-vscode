...
기능:
1. 구독     async def subscribe(self, symbol: str, data_type: WebSocketDataType,
            def add_global_handler(self, data_type: WebSocketDataType,
                           handler: Callable[[str, Dict[str, Any]], None]) -> None:              callback: Optional[Callable[[Dict[str, Any]], None]] = None) -> str: 관리 (구독/해지/상태 추적)
2. 자동 연결/재연결 처리
3. 데이터 수신 및 분배
4. 에러 처리 및 복구
"""
...
class WebSocketSubscriptionManager:
    ...
    async def _handle_ticker_data(self, message) -> None:
        """Ticker 데이터 처리"""
        try:
            ...
            # 개별 콜백 호출
            await self._dispatch_to_callbacks(symbol, WebSocketDataType.TICKER, data)

            # 전역 핸들러 호출
            await self._dispatch_to_global_handlers(WebSocketDataType.TICKER, symbol, data)

        except Exception as e:
            self._logger.error(f"Ticker 데이터 처리 실패: {e}")

    async def _handle_trade_data(self, message) -> None:
        """Trade 데이터 처리"""
        try:
            symbol = message.market
            data = message.data
            ...
            # 개별 콜백 호출
            await self._dispatch_to_callbacks(symbol, WebSocketDataType.TRADE, data)

            # 전역 핸들러 호출
            await self._dispatch_to_global_handlers(WebSocketDataType.TRADE, symbol, data)

        except Exception as e:
            self._logger.error(f"Trade 데이터 처리 실패: {e}")

    async def _handle_orderbook_data(self, message) -> None:
        """Orderbook 데이터 처리"""
        try:
            symbol = message.market
            data = message.data
            ...
            # 개별 콜백 호출
            await self._dispatch_to_callbacks(symbol, WebSocketDataType.ORDERBOOK, data)

            # 전역 핸들러 호출
            await self._dispatch_to_global_handlers(WebSocketDataType.ORDERBOOK, symbol, data)
