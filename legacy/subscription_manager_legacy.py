...
class TicketPool:
    ...
    def get_or_create_snapshot_ticket(self) -> Optional[str]:
        """스냅샷 전용 공용 티켓 획득 또는 생성"""
        # 스냅샷 전용 공용 티켓 키
        snapshot_key = "__snapshot_shared__"

        # 기존 스냅샷 티켓이 있으면 재사용
        if snapshot_key in self.data_type_mapping:
            ticket_id = self.data_type_mapping[snapshot_key]
            if ticket_id in self.active_tickets:
                logger.debug(f"기존 스냅샷 티켓 재사용: {ticket_id}")
                return ticket_id
            else:
                # 티켓이 없어졌으면 매핑 제거
                del self.data_type_mapping[snapshot_key]

        # 새 스냅샷 티켓 생성
        ticket_id = self.acquire_ticket("snapshot_shared")
        if not ticket_id:
            return None

        # 스냅샷 전용 설정
        ticket_info = self.active_tickets[ticket_id]
        ticket_info['is_only_snapshot'] = True
        ticket_info['shared_snapshot'] = True  # 공용 스냅샷 마킹
        ticket_info['subscribed_data_types'] = set()  # 구독된 데이터 타입 추적

        # 매핑 등록
        self.data_type_mapping[snapshot_key] = ticket_id

        logger.info(f"새 스냅샷 공용 티켓 생성: {ticket_id}")
        return ticket_id

    def is_snapshot_subscribed(self, data_type: str, symbols: List[str]) -> bool:
        """스냅샷 구독 상태 확인"""
        snapshot_key = "__snapshot_shared__"
        if snapshot_key not in self.data_type_mapping:
            return False
        ...
        ticket_info = self.active_tickets[ticket_id]
        subscribed_types = ticket_info.get('subscribed_data_types', set())
        
        # 데이터 타입과 심볼 조합으로 구독 상태 확인
        subscription_key = f"{data_type}:{':'.join(sorted(symbols))}"
        return subscription_key in subscribed_types

    def mark_snapshot_subscribed(self, data_type: str, symbols: List[str]) -> None:
        """스냅샷 구독 상태 마킹"""
        snapshot_key = "__snapshot_shared__"
        if snapshot_key not in self.data_type_mapping:
            return
        ...
        ticket_info = self.active_tickets[ticket_id]
        if 'subscribed_data_types' not in ticket_info:
            ticket_info['subscribed_data_types'] = set()
            
        subscription_key = f"{data_type}:{':'.join(sorted(symbols))}"
        ticket_info['subscribed_data_types'].add(subscription_key)
...
class SmartUnsubscriber:
    ...
    def create_hard_unsubscribe_request(cls, ticket_id: str) -> List[Dict[str, Any]]:
        """하드 해제 요청 생성 - 연결 재시작 방식 (빈 배열 방식 대신)

        Note:
        - 빈 배열 전송 시 업비트 API 에러로 연결 끊김 현상 발생
        - 대신 능동적 연결 종료 후 재연결 방식 사용
        - 이 메서드는 더 이상 실제 메시지를 반환하지 않음
        """
        # 실제로는 웹소켓 재연결을 통해 해제하므로
        # 임시 메시지만 반환 (사용되지 않음)
        return [
            {"ticket": ticket_id},
            {
                "type": "ticker",
                "codes": ["BTC-KRW"],  # 안전한 fallback 심볼
                "is_only_snapshot": True
            },
            {"format": "DEFAULT"}
        ]
...
class SubscriptionManager:
    ...
    def __init__(self,
        ...
        # 🚀 수정: 0도 유효한 값으로 처리 (None만 기본값 적용)
        snapshot_size = snapshot_pool_size if snapshot_pool_size is not None else 2
        realtime_size = realtime_pool_size if realtime_pool_size is not None else 2

        # 최소값 보장 (0은 허용하되 적어도 하나의 풀은 1 이상이어야 함)
        if snapshot_size == 0 and realtime_size == 0:
            logger.warning("모든 풀 크기가 0입니다. 최소 구독을 위해 스냅샷 풀을 1로 설정합니다.")
            snapshot_size = 1
    ...
    async def request_snapshot(self, data_type: str, symbols: List[str], **options) -> Optional[str]:
        ...
        # 🔧 수정: 스냅샷용 공용 티켓 재사용 또는 생성
        ticket_id = self.snapshot_pool.get_or_create_snapshot_ticket()
        if not ticket_id:
        ...
        subscription = {
            ...
            "mode": "snapshot",
            "created_at": datetime.now(),
            "message_count": 0,  # 메시지 카운트 초기화
            "is_only_snapshot": True,  # 스냅샷 전용 강제 설정
            **options
        }
        ...
        if self.snapshot_pool.add_subscription(ticket_id, subscription):
            ...
            # 🔧 이미 구독된 데이터인지 확인
            already_subscribed = self.snapshot_pool.is_snapshot_subscribed(data_type, symbols)
            
            if already_subscribed:
                # 이미 구독됨 - 메시지 전송 없이 콜백만 등록
                logger.debug(f"스냅샷 이미 구독됨: {data_type} - 메시지 전송 생략")
                logger.info(f"스냅샷 구독 재활용: {data_type} - {len(symbols)} symbols")
                return subscription_id
            else:
                # 새로운 구독 - 메시지 전송 필요
                try:
                    # 🔧 스냅샷 메시지 직접 생성 (현재 요청만)
                    snapshot_message = [
                        {"ticket": ticket_id},
                        {
                            "type": data_type,
                        ...
                        {"format": getattr(self.config, 'message_format', 'DEFAULT')}
                    ]

                    await self.websocket_connection.send(json.dumps(snapshot_message))
                    
                    # 구독 상태 마킹
                    self.snapshot_pool.mark_snapshot_subscribed(data_type, symbols)

                    logger.debug(f"스냅샷 메시지 전송: {snapshot_message}")
                    logger.info(f"스냅샷 구독 생성: {data_type} - {len(symbols)} symbols")
                    return subscription_id

            except Exception as e:
                logger.error(f"스냅샷 메시지 전송 실패: {e}")
                # 실패시 정리
                self.snapshot_pool.release_ticket(ticket_id)
                if subscription_id in self.active_subscriptions:
    ...
    async def unsubscribe(self, subscription_id: str, soft_mode: bool = True) -> bool:
        ...
        try:
            if soft_mode:
                ...
                # 하드 해제 - 능동적 재연결 방식
                logger.info(f"하드 해제 (재연결 방식): {subscription_id}")

                # 현재 구독을 제외한 다른 구독들만 보존
                preserve_list = [sub_id for sub_id in self.active_subscriptions.keys()
                                 if sub_id != subscription_id]

                # 재연결 및 선택적 복원 실행
                reconnect_result = await self.force_reconnect_and_restore(preserve_list)

                if reconnect_result["success"]:
                    logger.info(f"하드 해제 완료 - 재연결 성공: {reconnect_result}")
                else:
                    logger.error(f"하드 해제 실패 - 재연결 실패: {reconnect_result}")
                    return False

            # 구독 정보 백업 후 제거 (하드 모드의 경우 이미 정리됨)
            if soft_mode:
    ...
    async def force_reconnect_and_restore(self, preserve_subscriptions: Optional[List[str]] = None) -> Dict[str, Any]:
        """능동적 재연결 및 선택적 구독 복원

        Args:
            preserve_subscriptions: 보존할 구독 ID 목록 (None이면 모든 구독 보존)
        ...
        reconnect_result = {
            ...
            "total_before": len(self.active_subscriptions),
            "total_after": 0
        }

        try:
            # 1. 현재 활성 구독 백업 (선택적 보존)
            subscriptions_to_preserve = []
            if preserve_subscriptions is None:
                # 모든 구독 보존
                subscriptions_to_preserve = list(self.active_subscriptions.values())
                reconnect_result["preserved_subscriptions"] = len(subscriptions_to_preserve)
            else:
                # 지정된 구독만 보존
                for sub_id in preserve_subscriptions:
                    if sub_id in self.active_subscriptions:
                        subscriptions_to_preserve.append(self.active_subscriptions[sub_id])
                reconnect_result["preserved_subscriptions"] = len(subscriptions_to_preserve)

            # 2. 현재 구독 모두 정리 (메모리에서만)
            self.active_subscriptions.clear()
            self.snapshot_pool.active_tickets.clear()
            self.realtime_pool.active_tickets.clear()
            self.snapshot_pool.data_type_mapping.clear()
            self.realtime_pool.data_type_mapping.clear()

            # 3. WebSocket 재연결 요청
            if hasattr(self.websocket_connection, 'force_reconnect'):
                await self.websocket_connection.force_reconnect()
            elif hasattr(self.websocket_connection, 'disconnect') and hasattr(self.websocket_connection, 'connect'):
                await self.websocket_connection.disconnect()
                await asyncio.sleep(1)  # 잠시 대기
                await self.websocket_connection.connect()
            else:
                logger.warning("WebSocket 재연결 메서드를 찾을 수 없습니다")

            # 4. 보존된 구독 복원
            restored_count = 0
            failed_count = 0

            for subscription in subscriptions_to_preserve:
                try:
                    data_type = subscription["data_type"]
                    symbols = subscription["symbols"]
                    mode = subscription.get("mode", "realtime")
                    ...
                    if subscription_id:
                        restored_count += 1
                        logger.debug(f"구독 복원 성공: {data_type} - {symbols}")
                    else:
                        failed_count += 1
                        logger.warning(f"구독 복원 실패: {data_type} - {symbols}")
            ...
            reconnect_result.update({
                "success": True,
                "restored_subscriptions": restored_count,
                "failed_subscriptions": failed_count,
                "total_after": len(self.active_subscriptions)
            })

            logger.info(f"능동적 재연결 완료: 보존={reconnect_result['preserved_subscriptions']}, "
                        f"복원={restored_count}, 실패={failed_count}")

            return reconnect_result

        except Exception as e:
            logger.error(f"능동적 재연결 실패: {e}")
            reconnect_result["error"] = str(e)
            return reconnect_result
