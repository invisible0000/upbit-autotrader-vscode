"""
Domain Layer Logging - LEGACY VERSION
=====================================

이 파일은 성능 문제로 인해 레거시로 변경되었습니다.
Domain Events 기반 로깅이 272배 성능 저하를 일으켜서 단순한 인터페이스로 교체합니다.

성능 테스트 결과:
- Domain Events 방식: 39.73ms (10k 호출)
- 단순 함수 호출: 0.15ms (10k 호출)
- 성능 차이: 272.1배 느림

문제점:
1. 매 로그마다 UUID + datetime 생성
2. 불필요한 Domain Events 오버헤드
3. 복잡한 dataclass 생성 비용

해결책: 의존성 주입 기반 단순 인터페이스로 교체
"""
...
def create_domain_logger(component_name: str) -> DomainEventsLogger:
    ...
    Note:
        ...
        성능 문제로 사용 중단: 272배 느림 (39.73ms vs 0.15ms)
    """
