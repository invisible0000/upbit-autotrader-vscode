"""
DataUnifier V2 - 고급 데이터 통합 및 정규화 시스템 (Phase 1.3)
...
class DataNormalizer:
    ...
    def _validate_and_correct(self, ticker_data: TickerData) -> Tuple[List[str], List[str]]:
        ...
        # 현재가가 고저가 범위를 벗어나는 경우
        if ticker_data.current_price > ticker_data.high_24h:
            errors.append("현재가가 24시간 고가보다 높음")

        if ticker_data.current_price < ticker_data.low_24h:
            errors.append("현재가가 24시간 저가보다 낮음")
...
class IntelligentCache:
    ...
    def _is_expired(self, entry: DataCacheEntry) -> bool:
        """캐시 엔트리 만료 여부 확인"""
    ...
    def _estimate_cache_size(self) -> float:
        ...
        # 샘플 엔트리의 크기를 추정하여 전체 크기 계산
        sample_entry = next(iter(self._cache.values()))
        estimated_entry_size = len(str(sample_entry)) * 2  # 대략적인 바이트 크기
        total_size_bytes = estimated_entry_size * len(self._cache)

        return round(total_size_bytes / (1024 * 1024), 2)  # MB 단위
...
class DataUnifier:
    ...
    async def verify_data_consistency(self, symbol: str, time_window_minutes: int = 5) -> Dict[str, Any]:
        """
        데이터 일관성 검증 (Phase 1.3)

        Args:
            symbol: 검증할 심볼
            time_window_minutes: 검증 시간 윈도우 (분)

        Returns:
            Dict: 일관성 검증 결과
        """
        tracked_data = self._consistency_tracker.get(symbol, [])

        if len(tracked_data) < 2:
            return {"status": "insufficient_data", "message": "비교할 데이터가 부족합니다"}

        # 시간 윈도우 내 데이터 필터링
        cutoff_time = datetime.now() - timedelta(minutes=time_window_minutes)
        recent_data = [
            data for data in tracked_data
            if data.normalization_timestamp >= cutoff_time
        ]

        if len(recent_data) < 2:
            return {"status": "insufficient_recent_data", "message": "최근 데이터가 부족합니다"}

        # 일관성 분석
        price_variance = self._calculate_price_variance(recent_data)
        source_distribution = self._analyze_source_distribution(recent_data)
        quality_distribution = self._analyze_quality_distribution(recent_data)

        return {
            "status": "analyzed",
            "symbol": symbol,
            "analysis_period": f"{time_window_minutes}분",
            "data_points": len(recent_data),
            "price_variance_percent": float(price_variance),
            "source_distribution": source_distribution,
            "quality_distribution": quality_distribution,
            "consistency_score": self._calculate_consistency_score(recent_data)
        }
    ...
    def get_processing_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cache_hit_rate": (
                ...
                max(self._processing_stats["total_requests"], 1) * 100
            ),
            "error_rate": (
                self._processing_stats["error_count"] /
                max(self._processing_stats["total_requests"], 1) * 100
            )
        }
    ...
    async def warm_cache(self, symbols: List[str]) -> Dict[str, Any]:
        ...
        Returns:
            Dict: 예열 결과
        """
        self._logger.info(f"캐시 예열 시작: {len(symbols)}개 심볼")
        ...
        for symbol in symbols:
            try:
                # 간단한 REST API 호출로 캐시 예열
                mock_rest_data = {"market": symbol, "trade_price": 1000000}
                await self.unify_ticker_data(mock_rest_data, "rest")
                warmed_count += 1
            except Exception as e:
                error_count += 1
                self._logger.warning(f"캐시 예열 실패 ({symbol}): {e}")

        return {
            "total_symbols": len(symbols),
            "warmed_count": warmed_count,
            "error_count": error_count,
            "success_rate": warmed_count / len(symbols) * 100 if symbols else 0
        }
    ...
    def _calculate_price_variance(self, data_list: List[NormalizedTickerData]) -> Decimal:
        """가격 변동성 계산"""
        if len(data_list) < 2:
            return Decimal('0')

        prices = [data.ticker_data.current_price for data in data_list]
        avg_price = sum(prices) / len(prices)

        variance_sum = sum((price - avg_price) ** 2 for price in prices)
        variance = Decimal(str(variance_sum)) / Decimal(str(len(prices)))
        std_dev = variance.sqrt() if variance > 0 else Decimal('0')

        return (std_dev / avg_price * 100) if avg_price > 0 else Decimal('0')    def _analyze_source_distribution(self, data_list: List[NormalizedTickerData]) -> Dict[str, int]:
        """데이터 소스 분포 분석"""
        distribution = defaultdict(int)

        for data in data_list:
            distribution[data.ticker_data.source] += 1

        return dict(distribution)

    def _analyze_quality_distribution(self, data_list: List[NormalizedTickerData]) -> Dict[str, int]:
        """데이터 품질 분포 분석"""
        distribution = defaultdict(int)

        for data in data_list:
            distribution[data.data_quality.value] += 1

        return dict(distribution)

    def _calculate_consistency_score(self, data_list: List[NormalizedTickerData]) -> float:
        """일관성 점수 계산 (0.0 ~ 1.0)"""
        if len(data_list) < 2:
            return 1.0

        # 가격 변동성 기반 점수 (낮을수록 좋음)
        price_variance = float(self._calculate_price_variance(data_list))
        price_score = max(0, 1.0 - (price_variance / 10.0))  # 10% 변동성을 기준점으로
        ...
        avg_quality_score = sum(
            quality_scores[data.data_quality] for data in data_list
        ) / len(data_list)

        # 종합 점수 (가중 평균)
        final_score = (price_score * 0.6) + (avg_quality_score * 0.4)
