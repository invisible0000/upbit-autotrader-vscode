...
class EmptyCandleDetector:
    ...
    def _detect_gaps_in_datetime_list(
        ...
        # if api_end:
        #     api_end_prev_tick = TimeUtils.get_time_by_ticks(api_end, self.timeframe, -1)
        #     # 중복 방지: 같은 시간이 이미 존재하지 않는 경우에만 추가
        #     if api_end_prev_tick not in sorted_datetimes:
        #         sorted_datetimes.append(api_end_prev_tick)
        #         sorted_datetimes.sort(reverse=True)  # 다시 정렬 (업비트 내림차순 유지)
        #         logger.debug(f"🚀 마지막 Gap 감지용 시간점 추가: {api_end_prev_tick} (api_end={api_end}의 -1틱)")
        #     else:
        #         logger.debug(f"🔍 api_end-1틱이 이미 존재함: {api_end_prev_tick}")
