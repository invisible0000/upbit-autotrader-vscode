...
class LoggingManagementPresenter:
    ...
    def _on_apply_clicked(self):
        ...
        timestamp = datetime.now().strftime("%H:%M:%S")
        self.view.append_log(f"[{timestamp}] ⚙️ 환경변수 일괄 적용 시작...")

        # 환경변수 매니저를 통해 실제 적용
        applied_count = 0

        if self._pending_settings['log_level']:
            success = self._environment_manager.set_variable('UPBIT_LOG_LEVEL', self._pending_settings['log_level'])
            if success:
                self.view.append_log(f"  ✅ UPBIT_LOG_LEVEL: {self._pending_settings['log_level']}")
                applied_count += 1
            else:
                self.view.append_log("  ❌ UPBIT_LOG_LEVEL 설정 실패")
        ...
        if self._pending_settings['console_output'] is not None:
            ...
            success = self._environment_manager.set_variable('UPBIT_CONSOLE_OUTPUT', console_value)
            if success:
                self.view.append_log(f"  ✅ UPBIT_CONSOLE_OUTPUT: {console_value}")
                applied_count += 1
            else:
                self.view.append_log("  ❌ UPBIT_CONSOLE_OUTPUT 설정 실패")

        if self._pending_settings['log_scope']:
            success = self._environment_manager.set_variable('UPBIT_LOG_SCOPE', self._pending_settings['log_scope'])
            if success:
                self.view.append_log(f"  ✅ UPBIT_LOG_SCOPE: {self._pending_settings['log_scope']}")
                applied_count += 1
            else:
                self.view.append_log("  ❌ UPBIT_LOG_SCOPE 설정 실패")

        if self._pending_settings['component_focus']:
            component_value = self._pending_settings['component_focus']
            success = self._environment_manager.set_variable('UPBIT_COMPONENT_FOCUS', component_value)
            if success:
                self.view.append_log(f"  ✅ UPBIT_COMPONENT_FOCUS: {component_value}")
                applied_count += 1
            else:
                self.view.append_log("  ❌ UPBIT_COMPONENT_FOCUS 설정 실패")

        # 완료 메시지
        self.view.append_log(f"[{timestamp}] 🎯 환경변수 적용 완료: {applied_count}개 설정")
    ...
    def _on_clear_clicked(self):
        """로그 지우기 버튼 클릭 (로그 파일 + 콘솔 모두 지우기)"""
        self.view.clear_all()  # 로그 파일 + 콘솔 모두 지우기
        timestamp = datetime.now().strftime("%H:%M:%S")
    ...
    def _setup_infrastructure_logging(self) -> None:
        ...
        try:
            ...
            self._log_stream_capture.add_console_handler(self._on_console_output_received)
    ...
    def _on_console_output_received(self, console_messages: str) -> None:
        """콘솔 출력 수신 처리

        Args:
            console_messages: 배치로 전달된 콘솔 메시지들
        """
        if not self._is_real_logging_active:
            return

        try:
            # 여러 콘솔 메시지를 줄 단위로 분할하여 처리
            console_lines = console_messages.strip().split('\n')

            # 콘솔 출력 영역에 표시
            for console_line in console_lines:
                if console_line.strip():
                    self.view.append_console(console_line.strip())

            # 콘솔 출력 통계 업데이트 (디버깅용)
            stats = self._log_stream_capture.get_capture_stats()
            console_count = stats.get('total_console', 0)
            if console_count > 0 and console_count % 10 == 0:
                debug_msg = f"💻 콘솔 통계: {console_count}개 캡처됨"
                self.view.append_console(debug_msg)

        except Exception as e:
            error_msg = f"❌ 콘솔 출력 처리 오류: {e}"
            print(error_msg)  # 터미널에도 출력 (디버깅용)
            self.view.append_console(error_msg)
    ...
    def refresh(self) -> None:
        """탭 새로고침 처리 - 로깅 상태 및 콘솔 출력 갱신"""
        try:
            timestamp = datetime.now().strftime("%H:%M:%S")

            # 재귀 호출 방지를 위한 간단한 플래그 체크
            ...
            if self._is_real_logging_active:
                stats = self._log_stream_capture.get_capture_stats()
                status_msg = f"📊 [{timestamp}] 실시간 로깅 활성화 - {stats['total_logs']}개 로그, {stats.get('total_console', 0)}개 콘솔"
                self.view.append_log(status_msg)

                # 콘솔 출력 영역에도 상태 업데이트 (print 대신 직접 호출)
                console_msg = f"🔄 [{timestamp}] 탭 새로고침 - 콘솔 캡처 활성화"
                self.view.append_console(console_msg)

                # 환경변수 상태 다시 동기화
                self._sync_environment_variables()

            else:
                refresh_msg = f"🔄 [{timestamp}] 탭 새로고침 - 데모 모드"
                self.view.append_log(refresh_msg)

            # print 대신 직접 콘솔 영역에 추가 (무한 루프 방지)
            test_msg = f"🧪 [{timestamp}] 로깅 관리 탭 새로고침 완료"
            self.view.append_console(test_msg)

        except Exception as e:
            error_msg = f"❌ 새로고침 처리 오류: {e}"
            self.view.append_log(error_msg)
            # print도 제거하여 재귀 방지
