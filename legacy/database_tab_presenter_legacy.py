...
class DatabaseTabPresenter:
    ...
    def restore_database_backup(self, backup_id: str) -> bool:
        ...
        try:
            ...
            self.logger.info(f"📦 백업 복원 시작: {backup_id}")
            ...
            if not backup_file_path.exists():
                ...
                self.view.show_error_message("복원 실패", "백업 파일을 찾을 수 없습니다.")
                return False

            # 2. 백업 파일명에서 데이터베이스 타입 추출
            ...
            # 3. 대상 DB 파일 경로
            data_dir = Path("data")
            target_db_path = data_dir / f"{database_type}.sqlite3"

            # 4. 복원 전 현재 DB 백업 생성
            if target_db_path.exists():
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                safety_backup_name = f"{database_type}_before_restore_{timestamp}.sqlite3"
                safety_backup_path = user_backups_dir / safety_backup_name
            ...
            self.logger.info(f"✅ 백업 복원 성공: {backup_id} -> {target_db_path}")
            if hasattr(self.view, 'show_info_message'):
                self.view.show_info_message("복원 완료", f"{database_type} 데이터베이스가 성공적으로 복원되었습니다.")
            ...
            self.view.show_error_message("복원 실패", f"백업 복원 중 오류가 발생했습니다:\n{str(e)}")
            return False

    def delete_database_backup(self, backup_id: str) -> bool:
        """DDD Use Case를 사용한 데이터베이스 백업 삭제"""
        try:
            # DDD 컴포넌트 import
            ...
            self.logger.info(f"🗑️ DDD 백업 삭제 시작: {backup_id}")

            # Repository와 Use Case 초기화
            repository = DatabaseConfigRepository()
            use_case = DatabaseBackupManagementUseCase(repository)

            # Use Case 실행 (비동기 -> 동기 변환)
            import asyncio as async_loop
            loop = async_loop.new_event_loop()
            async_loop.set_event_loop(loop)
            try:
                success = loop.run_until_complete(use_case.delete_backup(backup_id))
            finally:
                loop.close()

            if success:
                self.logger.info(f"✅ DDD 백업 삭제 성공: {backup_id}")
                if hasattr(self.view, 'show_info_message'):
                ...
                self.logger.error(f"❌ DDD 백업 삭제 실패: {backup_id}")
                self.view.show_error_message("삭제 실패", "백업 삭제 중 오류가 발생했습니다.")
                return False

        except Exception as e:
            self.logger.error(f"❌ DDD 백업 삭제 실패: {e}")
