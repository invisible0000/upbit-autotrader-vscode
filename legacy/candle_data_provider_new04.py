"""
📋 CandleDataProvider New04 - 하이브리드 순차 청크 관리 방식
new01의 안정성 + new02의 순차 처리 결합

Created: 2025-09-12
Purpose: 사전 계획 최소화 + 실시간 순차 청크 확장 + 상태 관리
Features: 점진적 청크 생성, 중단/재시작 지원, 10 RPS 기반 예상 시간 제공
Philosophy: "최소 계획 + 실시간 확장 + 안정적 상태 관리"
"""
...
logger = create_component_logger("CandleDataProviderNew04")
...
class CandleDataProviderNew04:
    """
    하이브리드 순차 청크 관리 방식의 캔들 데이터 제공자
    ...
    def __init__(self, chunk_size: int = 200):
        ...
        logger.info("CandleDataProviderNew04 (하이브리드 순차 관리) 초기화 완료")
    ...
    def mark_chunk_completed(
        ...
        if state.total_collected >= state.total_requested:
            ...
            logger.info(f"✅ 전체 수집 완료: {request_id}, {state.total_collected}개")
            return True
