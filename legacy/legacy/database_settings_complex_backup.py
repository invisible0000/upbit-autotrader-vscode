...
기존의 잘 동작하던 DatabaseTabWidget을 기반으로 DDD/MVP 패턴을 적용했습니다.
...
from upbit_auto_trading.ui.desktop.screens.settings.presenters.database_tab_presenter import DatabaseTabPresenter
...
class DatabaseSettingsView(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.logger.info("📊 데이터베이스 설정 View (MVP) 초기화 시작")

        # UI 설정 (Presenter 생성 전에)
        self._setup_ui()

        # Presenter 초기화
        self.presenter = DatabaseTabPresenter(self)
        ...
        self.logger.info("✅ 데이터베이스 설정 View (MVP) 초기화 완료")
    ...
    def _on_restore_requested(self, backup_id: str):
        ...
        # 진행상황 표시
        self.show_progress("백업 복원 중...", 0)

        try:
            self.show_progress("백업 복원 중...", 50)
            self.logger.info(f"✅ 백업 복원 완료: {backup_id}")
            self.show_progress("백업 복원 완료!", 100)
            ...
            self.hide_progress()
            self.show_error_message("복원 실패", "백업 복원 중 오류가 발생했습니다.")
